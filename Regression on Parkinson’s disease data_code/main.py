...
def split_train_test(X, matricola):
    # Shuffle data  # todo: rimettere ignore index, a me non funziona per la versione di pandas
...
def main(matricola):
    par = 1 #not print the graphs of 20 seeds
    pd.set_option('display.precision', 3)
    plt.close('all')
    X = load_patients("parkinsons_updrs.csv")
    plot_cov(X)

    mm, ss, my, sy, regressors, Nf, X_tr_norm, X_te_norm, y_tr_norm, y_te_norm = split_train_test(X, matricola)
    ...
    # plots
    plot_weight_vector('LLS', Nf, w_hat, regressors)
    plot_weight_vector('SD', Nf, w_hatSD, regressors)
    ...
    plot_regression_line('LLS', y_te, y_hat_te)
    plot_regression_line('SD', y_te, y_hat_teSD)
    ...
    #results.to_csv('results_LLS.csv')
    #resultsSD.to_csv('results_SD.csv')
    ...
    for row in X_tr_norm:
        distances = np.linalg.norm(X_tr_norm - row, axis=1)
        nearest_neighbor_ids = distances.argsort()[:k + 1]  # take 1 more k nearest
        X_tr_local = X_tr_norm[nearest_neighbor_ids[1:]]  # discard the first point
    ...
    plot_regression_line('LOCAL', y_te, y_hat_te_local)
    ...
    resultsLocal.to_csv('results_LLR.csv')

    print("\n LOCAL result:\n", resultsLocal)

    # Test set MSE in relation to the change of K

    mse_local = []
    for k in range(10, 450, 20):
        y_hat_local = []
        for row in X_te_norm:
            distances = np.linalg.norm(X_tr_norm - row, axis=1)
            nearest_neighbor_ids = distances.argsort()[:k]
            ...
            w_local = SolveSteep(y_tr_local, X_tr_local)
            w_local.run(eps, Nit=Nit)
            y_tl, y_l = w_local.predict(row)
            y_hat_local.append(y_l * sy + my)

        y_hat_local = np.array(y_hat_local)
        E_te_local = (y_te - y_hat_local)
        resultsLocal = error_stats(E_trSD, y_tr, y_hat_tr_local, E_te_local, y_te, y_hat_local)
        mse_local.append(resultsLocal.loc["test"]['MSE'])

    plt.plot(list(range(10, 450, 20)), mse_local)
    plt.grid()
    plt.xlabel("K")
    plt.ylabel("MSE")
    plt.savefig('./K-MSE.png')
    plt.show()

    VarSeeds() #Test with 20 seeds

if __name__ == '__main__':
    # matricola = int(argv[1])
    matricola = 247586
    main(matricola)
